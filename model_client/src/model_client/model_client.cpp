...
void ModelClient::doModelClient(){
  bot_core_robot_urdf_t_subscription_t * sub = bot_core_robot_urdf_t_subscribe(lcm_, model_channel_.c_str(), robot_urdf_handler_aux, this);
  ...
  if (!keep_updated_) {
    ...
    bot_core_robot_urdf_t_unsubscribe(lcm_, sub);
  //  param->server_id = -1;
  }
}
...
void ModelClient::robot_urdf_handler(const char* channel, const bot_core_robot_urdf_t *msg){
